...
const MAX_BUFFER = 2; // keep only latest 2 per channel
const SECRET = process.env.DISCORD_POST_SECRET || "";

// One memory buffer per channel
globalThis.__DISCORD_CHANNELS__ = globalThis.__DISCORD_CHANNELS__ || {};
globalThis.__DISCORD_CLIENTS__ = globalThis.__DISCORD_CLIENTS__ || [];

const channels = globalThis.__DISCORD_CHANNELS__;
const clients = globalThis.__DISCORD_CLIENTS__;
...
function broadcastToClients(message) {
  ...
  globalThis.__DISCORD_CLIENTS__ = alive;
}
...
// === GET handler (SSE or latest snapshot) ===
  ...
  if (url.searchParams.get("stream") === "true") {
    const stream = new ReadableStream({
      start(controller) {
        ...
        // Initial data: all channels with last 2 messages
        const initial = {};
        for (const [ch, msgs] of Object.entries(channels))
          initial[ch] = msgs.slice(-MAX_BUFFER);

        controller.enqueue(
          `data: ${JSON.stringify({ type: "INITIAL_DATA", channels: initial })}\n\n`
        );

        req.signal.addEventListener("abort", () => {
          globalThis.__DISCORD_CLIENTS__ = globalThis.__DISCORD_CLIENTS__.filter(
            (c) => c !== client
          );
        });
      },
