...
export default function LiveDiscordUnified() {
  ...
  const authParam = searchParams.get("auth"); // for example, ?auth=xyz

// [
//   
  // ✅ Allowed URLs or tokens
  const ALLOWED_KEYS = [ "https://bot-1-8at8.onrender.com", "https://plantvsbrainrotstock.com"];
  const isAllowed = ALLOWED_KEYS.includes(authParam);

  useEffect(() => {
    if (!isAllowed) return; // prevent connecting if not allowed
